...
static ast::Type* p_Type(){
  ...
  // TODO:
  // 1. Match token `Int` using lookahead(TokenType t).
...
static ast::Statement* p_Statement(){
  ...
  if(isFirst[SymbolType::Expression][next_token.type]){
    ...
    // else if (...) {}
      ...
      mind::err::issue(next_token.loc, new mind::err::SyntaxError("expect if/LBRACE/SEMICOLON, get" + TokenName[next_token.type]));
      return nullptr;
    }
  }
...
static ast::Expr* p_Assignment(){
  ...
  // p_declaration p_block p_if p_return p_type
    lookahead(TokenType::ASSIGN);
    ast::Expr* expression = p_Expression();
    return new ast::AssignExpr(dynamic_cast<ast::LvalueExpr*>(node)->lvalue, expression, node->getLocation());
  }
  return node;
}
...
static ast::Expr* p_Relational(){
  ...
  while(next_token.type == TokenType::LT || next_token.type == TokenType::GT || next_token.type == TokenType::LEQ || next_token.type == TokenType::GEQ){
